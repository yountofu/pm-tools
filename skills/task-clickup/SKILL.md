...
태스크 생성 시 유저가 타입을 명시하면 (`[Epic]`, `[UI]`, `[Docs]` 등) 그대로 진행한다.
**타입을 명시하지 않고** "태스크 만들어 줘"라고만 하면, 반드시 물어본다:
...
PR이 올라가거나 대화 중 ClickUp 관련 내용이 나오면, 어떤 작업을 원하는지 유저에게 먼저 확인한다:

> "ClickUp 태스크도 업데이트할까요? 어떤 작업이 필요한가요?"
> - 상태 변경 (진행중 → 리뷰 등)
> - PR 링크 첨부
> - description 업데이트
> - 코멘트 추가
...
2. 태스크명 형식: `[UI] {Epic 내용}` 또는 `[Docs] {Epic 내용}`
...
태스크 타입별로 다른 description 템플릿을 사용한다.

| 타입 | 템플릿 | 예시 | 용도 |
|------|--------|------|------|
| `[Epic]` | [description-epic.md](templates/description-epic.md) | [예시](examples/description-epic.md) | 기능 단위 상위 태스크. 접근 경로, 주요 기능, 서브태스크 구성 |
| `[UI]` | [description-ui.md](templates/description-ui.md) | [예시](examples/description-ui.md) | UI 구현 서브태스크. 작업 요약 + 접근 경로 + 스크린샷 |
| `[Docs]` | — | — | Policy 내용 간추려서 작성 (별도 템플릿 없음) |
| 업데이트 | [description-update.md](templates/description-update.md) | [예시](examples/description-update.md) | 기존 태스크 변경 시. before/after + 영향 범위 |
| 서브태스크 (상세) | [description-ticket.md](templates/description-ticket.md) | [예시](examples/description-ticket.md) | spec 요약 + 스크린샷 포함 상세 버전 |
...
[templates/description-ticket.md](templates/description-ticket.md) 참조.
작성 예시: [examples/description-ticket.md](examples/description-ticket.md)
